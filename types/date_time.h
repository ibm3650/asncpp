...
class date_time_t
        : public asn1_type<std::tm, static_cast<uintmax_t>(asn1_tag::DATE_TIME)> {
public:
    //TODO: Add support for fractional seconds
    //TODO: Default constructor set local time
    date_time_t() = default;

    explicit date_time_t(std::tm data) {
        this->_value = data;
    }
...
private:
    [[nodiscard]] std::string _to_string() const {
        constexpr size_t buffer_size = std::size("YYYY-MM-DDTHH:MM:SS");
